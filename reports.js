document.addEventListener('DOMContentLoaded', function() {
    ...
    function generateCategoryReport(monthYear) {
        ...
        // تجهيز بيانات المخطط
        const categoryNames = [
            ...Object.keys(categories.income).map(name => `مدخول - ${name}`),
            ...Object.keys(categories.expense).map(name => `مصروف - ${name}`)
        ];
    ...
    function updateCategoryTable(categories) {
        ...
        Object.entries(categories.income).forEach(([category, amount]) => {
            ...
            row.innerHTML = `
                ...
                <td>${((amount / Object.values(categories.income).reduce((a, b) => a + b, 0)) * 100}%</td>
            `;
            reportTable.appendChild(row);
        });
        ...
        Object.entries(categories.expense).forEach(([category, amount]) => {
            ...
            row.innerHTML = `
                <td>${getCategoryName(category)}</td>
                <td>${amount.toFixed(2)} جنيه</td>
                <td>${((amount / Object.values(categories.expense).reduce((a, b) => a + b, 0)) * 100}%</td>
            `;
            reportTable.appendChild(row);
        });
    }
